...
export function buildForTaxon(taxonId, { seed = 7 } = {}) {
  ...
  // ten legs alone is not a myriapod to the renderer; it wants the long body too
  if (node.clade === 'myriapod' && !window.body_length) g.body_length = Math.max(g.body_length, 0.85);
  if (node.clade === 'arachnid' && !window.body_length) g.body_length = Math.min(g.body_length, 0.6);
