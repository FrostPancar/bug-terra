...
export const PARTS = [
  ...
  },
  {
    ...
    name: 'Leg colour',
    core: true,
    blurb: 'Normal (the body\'s own limb tone), inked near-black, or a gradient running from a colour of its own at the foot to the normal limb tone up near the body.',
    drawnBy: 'palette() → col.limb / drawLegs()',
    gate: 'pattern_leg selects: 0 normal, 1 inked, 2 gradient',
    variantGene: 'pattern_leg',
    variants: variants(['normal', 'inked', 'gradient'], [
      'the body\'s own limb tone — the default',
      'near-black legs, antennae and tail',
      'a colour of its own at the foot, fading to the normal limb tone toward the body',
    ]),
    variantOf: (g) => Math.max(0, Math.min(2, Math.round(g.pattern_leg ?? 0))),
    setVariant: (_g, i) => ({ pattern_leg: i }),
    genes: [
      { gene: 'pattern_leg', effect: 'was a continuous 0-1 threshold ("above 0.5 the legs ink") — a slider implying a spectrum between two states nothing in between meant anything. Stepped 0/1/2 now: normal/inked/gradient' },
      { gene: 'pattern_leg_hue', effect: 'the gradient mode\'s foot-end colour, off the reference palette. Unused unless pattern_leg selects gradient' },
    ],
  },
  {
    id: 'hornpattern',
